...
事件：
devicechange //可用的多媒体设备列表改变时触发,可以使用enumerateDevices()获取设备的更新列表
```
...
```javascript
定义：const mediaSession = navigator.mediaSession
属性：mediaSession.metadata           //返回当前媒体会话的元数据
     mediaSession.playbackState      //返回当前媒体会话是否处于播放状态
方法：mediaSession.setActionHandler() //设置一个监听当前媒体会话的动作句柄
