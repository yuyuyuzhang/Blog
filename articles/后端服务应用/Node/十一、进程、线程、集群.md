...
属性：操作系统属性：
     ...
     父子进程通信属性：
     process.channel                                   //返回当前Node进程通往父进程的通道
     process.connected                                 //返回当前Node进程通往父进程的通道是否仍连接
方法：身份标识方法：
     ...
     process.disconnect()                              //无返回值,关闭当前Node进程通往父进程的通道    
```
...
disconnect               //当前 Node 进程通往父进程的通道关闭时触发
...
方法：衍生异步子进程：
     ...
     child_process.fork(jsModulePath,[args],[options])                           //返回ChildProcess实例,当前Node进程使用父进程的 process.execPath 衍生新的子进程
     衍生同步子进程：
...
* 主进程创建监听套接字，并将其发送给感兴趣的子进程，然后子进程直接接收传入的连接，理论上该方法具有最好的性能，但实践中由于操作系统调度机制难以琢磨，分发往往非常不平衡，可能出现 8 个子进程中的 2 个子进程分担了所有连接超过 70% 的负载的情况
...
属性：
     ...
     cluster.workers                  //
方法：
     cluster.disconnect([cb])         //无返回值,集群所有工作进程断开连接,完成时调用可选参数cb
     cluster.fork([env])              //返回并衍生新的子进程,并将可选参数env添加到衍生子进程的环境变量
