...
Android、iOS、最新的 Windows 系统都可以`随时自主的停止后台进程`，及时释放系统资源，也就是说文档随时可能被系统丢弃掉，以前的浏览器 API 未考虑到这种情况，导致开发者无法监听到系统丢弃页面
...
* **Passive 阶段**：文档可见，但没有输入焦点，该阶段只可能发生在桌面同时拥有`多个浏览器 Tab 窗口或 APP 窗口`的情况下
...
* **Terminated 阶段**：Hidden 阶段之后，`用户主动关闭 Tab 窗口、用户在当前 Tab 窗口前往其他文档、用户直接在任务管理器中清除浏览器进程`，导致当前文档开始被浏览器卸载并从内存中清除
...
* **DOMContentLoaded**：文档形成完整的 DOM 树后触发，不理会图片、CSS、JS 文件等其他资源是否已经加载完毕
* **load**：文档及所有其他资源加载成功后触发
* **pageshow**：用户加载当前文档时触发，该事件与页面可见性无关，只与浏览器 `History 记录的变化`有关
* **focus**：文档获得焦点时触发
* **blur**：文档失去焦点时触发
* **visibilitychange**：文档可见状态变化时触发
* **freeze**：文档进入 Frozen 状态时触发，这个事件的回调函数最长只能运行 `500ms`，并且只能复用已打开的网络链接，不能发起新的网络请求
* **resume**：文档离开 Frozen 状态复苏时触发
* **beforeunload**：文档即将卸载时触发，该事件发生时，文档仍然可见，卸载仍可取消
* **pagehide**：用户离开当前文档时触发，该事件与页面可见性无关，只与浏览器 `History 记录的变化`有关
* **unload**：文档正在卸载时触发
...
* 文档将要卸载前 beforeunload 事件在 `window 对象上`触发
...
* 如果文档处于 Active、Passive、Hidden 状态
