...
* 若一个操作数是 undefined、null、布尔值，另一个操作符不是字符串，则将当前操作数`转换成数值`，执行数值加法
...
* 若两个操作数都是数值，执行常规数值减法
  ...
  * +0 - -0 = +0
  * -0 - +0 = -0
* 若一个操作符是 undefined、null、布尔值、字符串，调用 `Number()` 方法将其转换为数值，执行数值减法
* 若一个操作数是对象，调用对象的 `valueOf()`、没有则调用 `toString()`，然后再`转换成数值`，执行上述规则
...
* 若一个操作符是布尔值，先将其转换为数值，再执行数值比较
* 若一个操作数是数值，则将另一个操作符转换为数值，再执行数值比较
...
**③** break 关键字会使`执行流跳出switch语句`，省略break会导致执行完当前 case，继续执行下一个 case
