...
使用正则表达式字面量和使用构造函数创建的正则表达式是一样的，每次都创建新的RegExp实例，由于书写简单，常用字面量形式
...
第一个参数reg不设置标志g，替换`第一个`匹配项，设置标志g，替换`所有`匹配项
...
① 字符组（`[]`）：只匹配字符组中任意一个字符，如[abc]表示匹配a、b、c中任意一个字符 

② 范围字符组（`[-]`）：只匹配范围字符组中任意一个字符，如[0-9a-zA-Z]表示匹配任意一个数字或不区分大小写字母

③ 排除字符组（`[^]`）：只匹配排除字符组外任意一个字符，如[^abc]表示匹配除a、b、c外的任意一个字符
...
ES6引用具名组匹配 `?<组名>`，允许为每组匹配指定一个组名，既便于理解，又便于引用，可以从 `exec()、match() 方法返回值的 groups 属性`上获取每组

#### ① API中引用 

在实现环境的 API 中引用之前出现过的分组，`$i`引用第i个分组，`$<组名>`引用指定组名的分组
...
在正则中引用之前出现过的分组，`\i`引用第i个分组，`\k<组名>`引用指定组名的分组
...
非捕获括号`(?:p)`，只分组而不引用，既不在API中引用，也不在正则中反向引用
