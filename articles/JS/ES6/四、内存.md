...
对象在堆内存中以 Hash 结构存储，对象的属性被表示为多个 key-value  键值对，key 的数据类型是字符串，value 的数据类型

对象在堆内存中以 <String, Object>类型的 HashMap 结构存储

对象属性的三种类型

* 存取器属性

* 数据属性：基本数据类型属性、引用数据类型属性
...
```javascript
const arr = ['张三', {value: 1}, {value: [1, 2]}];
```
...
程序运行时，操作系统给每个进程分配一个堆，每个线程分配一个栈，堆的大小不确定，需要的话可以不断增加，栈的大小固定，超出限制则会发生栈溢出，JS是单线程语言，每个进程只有一个线程 
...
① 引用数据类型值`大小不固定`，在堆内存中存储，由于结构复杂，数据量大，在堆内存中表现为`一段连续的内存地址`，
