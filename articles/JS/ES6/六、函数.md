...
**④** 没有 callee 对象、arguments 对象，只能使用`rest参数`
...
ES6 指定参数默认值之后，length 属性返回`函数未指定默认值的形参个数(不包含rest参数)`
...
函数的作用域方法通常用于`改变函数的this指向`

#### ① function函数

this 指向 `function函数运行时所在的作用域`
...
没有 this，引用外层的 this，指向`箭头函数定义时所在的作用域`，因而无法使用函数的作用域方法改变this指向，ES6 新增`函数绑定运算符::`，用来替代函数的作用域方法，改变箭头函数的 this 指向
...
function 函数有自己的 this，function 函数自身的 this 指向`function函数调用时所在的执行上下文`
...
箭头函数没有自己的 this，引用`距离最近的执行上下文的this`，箭头函数中的 this 指向`箭头函数定义时所在的执行上下文`
