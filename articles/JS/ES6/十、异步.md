...
同步执行是`阻塞`的：立即执行操作，等待返回结果后再继续执行后面的代码
...
异步执行是`非阻塞`的：立即执行操作，而后继续执行后面的代码，未来某个时刻返回结果
...
① 事件、MutationObserver、AJAX 异步请求、定时器等异步操作都是采用回调函数的方式：立即执行操作，而后继续执行后面的代码，`未来某个时刻返回结果时将回调函数添加到 JS 引擎线程的等待队列`
...
//立即执行代码开始监听，而后继续执行后续代码，未来事件发生时将copy事件回调函数添加到JS引擎线程的等待队列
```
...
//立即执行代码开始观察页面DOM的变动，而后继续执行后续代码，未来DOM发生变动时将回调函数添加到JS引擎线程的等待队列
```
...
//立即执行代码发出请求，而后继续执行后续代码，未来返回数据时将readystatechange回调函数添加到JS引擎线程的等待队列
```
...
JS 引擎线程作为一个单线程为什么能够实现异步？因为标签页渲染进程除了 JS 引擎线程，还有事件触发线程、网络请求线程、定时器触发线程，将一些异步操作交给其他线程处理，然后采用`事件循环机制`处理返回结果

#### ① 标签页渲染进程的各个线程

浏览器会为`每个标签页`单独启动一个渲染进程，渲染进程主要负责将 HTML、CSS、JS 转化为用户可以与之交互的网页，每个渲染进程都会启动单独的渲染引擎线程、JS 引擎线程、事件触发线程、定时器触发线程、网络请求线程
...
**原理**：`微任务队列的优先级高于宏任务队列`，每次事件循环时会从微任务队列中获取任务，只有当微任务队列`为空`时才会从宏任务队列中获取任务，同一个队列中的任务只遵循`先进先出`的原则

* **微任务队列**：Promise.then/catch/finally()、MutaionObserver 回调函数
* **宏任务队列**：事件回调函数、HTTP 异步请求回调函数、定时器回调函数
...
![事件循环机制](../../../images/JS/语法/异步/事件循环机制.png)
...
![JS引擎线程执行流程示例](../../../images/JS/语法/异步/JS引擎线程执行流程示例.png)
...
* then 方法的参数函数返回`值`
  ...
  then 方法返回的新 Promise 实例的状态为`已成功(fulfilled)`，then 方法de 参数函数返回值传递给新 Promise 实例的 then 方法的参数函数
...
for-of 循环可以自动遍历 Generator 函数生成的遍历器对象，一旦 next 方法返回对象的 done 属性为 true，for-of 循环就会终止，且不包含该返回对象，因此 `return 语句不在 for-of 循环中`
...
JS 引擎执行某个 Generator 函数时，创建该 Generator 函数的执行上下文，并且压入执行上下文栈，一旦遇到 yield 命令，Generator 函数上下文就从执行上下文栈中弹出但并不销毁而是暂时`冻结`在当前状态，等到对该 Generator 函数的遍历器对象执行 next 命令时，该 Generator 函数上下文重新压入执行上下文栈，冻结的变量和对象恢复执行
