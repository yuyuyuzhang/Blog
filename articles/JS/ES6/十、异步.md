...
//立即执行代码开始监听，而后继续执行后续代码，未来事件发生时将copy事件回调函数添加到JS引擎线程的任务队列
```
...
//立即执行代码开始观察页面DOM的变动，而后继续执行后续代码，未来DOM发生变动时将回调函数添加到JS引擎线程的任务队列
```
...
//立即执行代码发出请求，而后继续执行后续代码，未来返回数据时将readystatechange回调函数添加到JS引擎线程的任务队列
```
...
JS 引擎线程作为一个单线程为什么能够实现异步？因为标签页渲染进程除了 JS 引擎线程，还有事件触发线程、异步请求线程、定时器计数线程，将一些异步操作交给其他线程处理，然后采用`事件循环机制`处理返回结果

#### ① 标签页渲染进程的各个线程

浏览器会为`每个标签页`单独启动一个渲染进程，渲染进程主要负责将 HTML、CSS、JS 转化为用户可以与之交互的网页，每个渲染进程都会启动单独的渲染引擎线程、JS 引擎线程、事件触发线程、定时器计数线程、异步请求线程
