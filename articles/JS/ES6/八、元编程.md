...
① 将ES5中Object对象的一些`明显属于语言内部的方法`移植到Reflect对象上，并修改这些方法的返回结果，让其更合理

② 让Object操作都变成`函数行为`，某些Object操作是命令式

③ Reflect对象的方法与Proxy对象的方法一一对应，使得Proxy对象可以方便地调用Reflect对象的方法来完成默认行为，作为修改行为的基础
