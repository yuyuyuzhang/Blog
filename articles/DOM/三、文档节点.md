...
文档状态(不推荐使用,被进度事件取代)：
document.readyState        //返回当前文档的加载状态(loading:正在加载文档,complete:文档加载完成)
文档可见：
document.hidden            //返回布尔值,表示当前文档是否可见
document.visibilityState   //返回当前文档可见状态
                           //(visiable:文档可见,hidden:文档不可见,prerender:文档处于渲染状态,unloaded:文档从内存中卸载)
文档域名：
...
document.writeIn(txt)                 //无返回值,当前文档加载过程中动态追加内容,内容尾部添加换行符
创建节点：
...
DocumentType对象表示`文档类型`，在浏览器中并不常用，`仅有少数浏览器支持`
...
DocumentFrament对象表示`一个文档片段`，DOM规定文档片段是一种轻量级的文档，可以包含和控制节点，但不会像完整的文档那样占用额外的资源 

**优点**：文档片段`不属于当前文档`，故操作文档片段节点比直接操作DOM树快得多，常用于先构建一个DOM树结构，然后再插入当前文档，可以`避免浏览器反复渲染新信息`的问题
